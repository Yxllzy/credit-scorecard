...
# ============================================================
# 1. 切分训练集 / 测试集（防泄露第一步！）
#    test_size=0.2 → 20%留作考试
#    stratify=y    → 保证训练/测试里坏人比例一致(都约6.7%)
#    random_state  → 固定随机种子，保证每次切分结果一样(可复现)
# ============================================================
...
def fit_woe(x_train, y_train, feature, manual_bins=None, q=5):
    ...
    woe_map = g['woe'].to_dict()        # 每个箱区间 → WOE值
...
for f in features:
    ...
    # 关键：训练集和测试集都用【从训练集学到的】edges和woe_map
