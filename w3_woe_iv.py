...
target = 'SeriousDlqin2yrs'   # 需要预测的目标列（1=坏人）
features = [c for c in df.columns if c != target]   

# 定义函数：对单个字段做分箱 + 算 WOE / IV
def calc_woe_iv(data, feature, target, bins=5):
    ...
    # 用 qcut 按分位数分箱（让每箱人数尽量均匀）；duplicates='drop' 防止边界重复报错
    try:
        df_temp['bin'] = pd.qcut(df_temp[feature], q=bins, duplicates='drop')
    except Exception:
        # 对于某些字段取值太集中无法分箱的，采用普通等宽分箱
    ...
    # 每箱中：坏人占总坏人比例、好人占总好人比例；+0.5 的目的是达到拉普拉斯平滑，即防止某箱坏人数为0时除零出错
